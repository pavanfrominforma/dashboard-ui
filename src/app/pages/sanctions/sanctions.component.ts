...
export class SanctionsComponent implements OnInit, AfterViewChecked {
  ...
  loadFeedsData() {
    this.isLoading = true;
    this.apiService.getSanctionsFeeds(this.filters).subscribe({
        next: (response: any) => {
  ...
  loadFeedsCount() {
    ...
    this.apiService.getSanctionsFeedStatusCount().subscribe({
        next: (response: any) => {
            const feed = response;
            this.selectedFeedCount = {
              vslStatus: this.preprocessStatusCounts(response?.vslStatus),
              psnStatus: this.preprocessStatusCounts(response?.psnStatus),
              comStatus: this.preprocessStatusCounts(response?.comStatus)
            }
            console.log(this.selectedFeedCount)
  ...
  preprocessStatusCounts(feed: any){
    return {
      active: prefixZero(feed?.active || 0),
      inactive: prefixZero(feed?.inactive || 0),
      due: prefixZero(feed?.due || 0)
    }
  }
